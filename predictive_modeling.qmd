...
Data Cleaning - Convert 'Date' column to date type - Convert remaining 3 categorical variables to factors - Rename columns to be consistent & easy to use
...
Create LASSO model instance and workflow - Use 'penalty = tune()' to indicate that we are going to be tuning the model - use 'mixture = 1' to turn this into a LASSO model rather than elastic net
...
Each tuning value has the same RMSE, so use a small number of levels to shorten run time
...
Finalize the workflow with the best model and fit to the training data
...
Tuning all 3 parameters takes a very long time to run, so ran it once and then specified the tree_depth and min_n based off of those results.
...
Finalize the workflow and fit the best model on the full training set
...
The random forest model has the lowest RMSE and MAE out of the 5 models.
...
rbind(
  ...
  bag_final |> 
    collect_metrics() |> 
    mutate(Model = "BAGGED", .before = ".metric"),
  rf_final |>
    collect_metrics() |> 
